...
export default function WeatherWidget({
  ...
  return (
    <div className={`flex flex-col gap-3 rounded-2xl bg-white border border-slate-200 p-4 sm:p-5 shadow-xs transition-all ${className}`}>
      {/* Top Header: Title & Hazard Alert */}
      <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
        <div className="flex items-center gap-2">
          <span className="text-base sm:text-lg">🌤️</span>
          <h3 className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-cyan-600">
            {t('weather_hazards')}
          </h3>
        </div>
        {current.condition && (
          <span className="text-[10px] sm:text-xs font-semibold capitalize px-2 sm:px-2.5 py-0.5 rounded-full bg-slate-100 text-slate-700 border border-slate-200">
      ...
      <div className="flex flex-col xs:flex-row items-start xs:items-center justify-between gap-3 sm:gap-4">
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-4 text-left xs:text-right w-full xs:w-auto">
          <div className="bg-slate-50 xs:bg-transparent p-2 xs:p-0 rounded-lg border border-slate-200 xs:border-0">
            <span className="text-[9px] sm:text-[10px] text-slate-500 block uppercase font-bold">{t('humidity')}</span>
            <span className="text-xs sm:text-sm font-bold text-slate-800">{current.humidity}%</span>
          </div>
          <div className="bg-slate-50 xs:bg-transparent p-2 xs:p-0 rounded-lg border border-slate-200 xs:border-0">
            <span className="text-[9px] sm:text-[10px] text-slate-500 block uppercase font-bold">{t('wind')}</span>
            <span className="text-xs sm:text-sm font-bold text-slate-800">{Math.round(current.windSpeed)} km/h</span>
          </div>
          <div className="bg-slate-50 xs:bg-transparent p-2 xs:p-0 rounded-lg border border-slate-200 xs:border-0">
            <span className="text-[9px] sm:text-[10px] text-slate-500 block uppercase font-bold">{t('rainfall')}</span>
            <span className="text-xs sm:text-sm font-bold text-slate-800">{current.precipitation} mm</span>
          </div>
        </div>
      </div>
      ...
      {!compact && forecast && forecast.length > 0 && (
        <div className="mt-1 pt-2.5 border-t border-slate-100">
          <div className="text-[10px] sm:text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">
            24-Hour Forecast
          </div>
          <div className="flex gap-2 sm:gap-2.5 overflow-x-auto pb-1.5 scrollbar-thin">
            {forecast.slice(0, 8).map((pt, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="flex flex-col items-center justify-center p-2 rounded-xl bg-slate-50 min-w-[54px] sm:min-w-[58px] text-center border border-slate-200 shrink-0"
                >
                  <span className="text-[9px] sm:text-[10px] font-semibold text-slate-500">{pt.time}</span>
                  ...
                  {pt.precipitation > 0 && (
                    <span className="text-[8px] sm:text-[9px] font-bold text-cyan-700 bg-cyan-50 px-1 py-0.5 rounded border border-cyan-200 mt-0.5">
                      {pt.precipitation}mm
                    </span>
                  )}
                </div>
