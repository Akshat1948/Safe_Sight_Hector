...
export default function SOSButton({ siteId, className = '', onSosDispatched }: SOSButtonProps) {
  ...
  return (
    <div className={`flex flex-col items-center justify-center ${className}`}>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center p-3 sm:p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
          <div className="w-full max-w-md p-4 sm:p-6 rounded-2xl bg-slate-900 border border-red-500/50 shadow-2xl text-white space-y-3 sm:space-y-4 max-h-[90vh] overflow-y-auto">
            {sosStatus === 'success' ? (
              <div className="text-center space-y-3 py-4">
                ...
                <h3 className="text-xl font-bold text-white">{t('sos_dispatched')}</h3>
                ...
                <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 text-left text-xs font-mono space-y-1">
                  ...
                  <div className="flex justify-between text-slate-400">
                    <span>{t('status')}:</span>
                    <span className="text-emerald-400 font-bold uppercase">{t('dispatched_queued')}</span>
                  </div>
                  <div className="flex justify-between text-slate-400">
              ...
              <>
                ...
                <div className="space-y-3 font-mono text-xs">
                  <div>
                    <label className="block text-slate-400 mb-1">{t('emergency_context')}:</label>
                    <textarea
                      rows={2}
                      value={message}
                  ...
                  <div>
                    <label className="block text-slate-400 mb-1">{t('contact_phone')}:</label>
                    <input
                      type="tel"
                      value={contactPhone}
