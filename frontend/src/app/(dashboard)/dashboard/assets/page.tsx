...
export default function AssetTrackingPage() {
  ...
  useEffect(() => {
    ...
    getZones(user.siteId).then((res) => {
      if (res.success && res.data) setZones(res.data);
    });

    Promise.all([getParkingStatus(user.siteId), getShuttleStatus(user.siteId)]).catch(console.error);
  }, [user?.siteId]);
        ...
        <div className="flex flex-col md:flex-row h-[calc(100vh-120px)] min-h-[560px] gap-4 overflow-hidden">
          ...
          <div className="w-full md:w-[55%] lg:w-[60%] bg-surface-container-low border border-border-subtle rounded-lg relative overflow-hidden flex flex-col shadow-sm">
            ...
            <div className="flex-1 w-full h-full relative bg-surface-container">
              ...
              {/* Simulated Tactical Route Overlay SVG */}
              <svg className="absolute inset-0 w-full h-full pointer-events-none z-[400]" style={{ overflow: 'visible' }}>
                <path
                  d="M 280 200 L 360 260 L 480 220 L 580 340"
                  fill="none"
                  stroke="#855300"
                  strokeDasharray="6 4"
                  strokeWidth="2.5"
                ...
                />
                <circle cx="580" cy="340" r="6" fill="#dc2626" className="radar-pulse" />
              </svg>

              {/* Selected Asset Detail Overlay (Glassmorphism Light) */}
